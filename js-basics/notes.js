...
** функция => хойстинг;
** глобальная область => хойстинг + добавляется в window (window.someVar) => становится глобальной.
...
* 1 непримитив:
** Object
* Функция (Function) - особый, вызываемый объект.
* Для экземпляров встроенных объектов Array, Date, String, Number, Boolean и др. typeof вернет 'object'.
* Для экземпляров вместо оператора typeof используется instanceof - проверка наследования =>
*/
// => если класс отнаследован от какого-либо, то его экземпляры являются наследниками последнего =>
log([] instanceof Array && [] instanceof Object); // true, т. к. Object - прототип Array
// т. к. [].__proto__.__proto__.constructor.name === 'Object'
